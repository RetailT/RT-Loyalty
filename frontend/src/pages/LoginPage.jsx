...
export default function LoginPage({ onNavigate }) {
  ...
  // Start/stop QR scanner when tab changes
  useEffect(() => {
    if (tab === 'qr') {
      startScanner();
    } else {
  ...
  const handleQrLogin = async (qrCode) => {
    ...
    setQrError('');
    try {
      const res = await fetch(`${API}/api/portal/auth/qr-login`, {
        method: 'POST',
      ...
      setQrError(e.message || 'Invalid QR code.');
      setQrResult('');
      setQrLoading(false);
    ...
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '24px 16px' : '48px 16px' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: 20, padding: isMobile ? '28px 20px' : '36px 32px', boxShadow: '0 24px 80px rgba(0,0,0,0.12)' }}>
          ...
          {/* Tab switcher */}
          {step === 'email' && (
            <div style={{ display: 'flex', background: mode === 'dark' ? '#1a1a1a' : '#f0f0f0', borderRadius: 12, padding: 4, marginBottom: 24, gap: 4 }}>
              {[
                { key: 'email', label: '📧 Email OTP' },
                { key: 'qr',    label: '📷 Scan QR'   },
              ].map(t => (
                <button key={t.key} onClick={() => { setTab(t.key); setError(''); setQrError(''); }}
                  style={{
                    flex: 1, padding: '9px 8px', border: 'none', borderRadius: 9, cursor: 'pointer',
                    background: tab === t.key ? (mode === 'dark' ? '#2a2a2a' : '#fff') : 'transparent',
                    color: tab === t.key ? '#FF6B00' : theme.textMuted,
          ...
          {tab === 'qr' && (
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: theme.textMuted, fontSize: 11, fontFamily: "'Space Mono',monospace", marginBottom: 16, lineHeight: 1.6 }}>
                Point your camera at your membership QR code
              </p>
              ...
              <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', background: '#000', marginBottom: 16 }}>
                ...
                {scanning && !qrLoading && (
                  <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                    {[
                      { top: 16, left: 16, borderTop: '3px solid #FF6B00', borderLeft: '3px solid #FF6B00' },
                      { top: 16, right: 16, borderTop: '3px solid #FF6B00', borderRight: '3px solid #FF6B00' },
                      { bottom: 16, left: 16, borderBottom: '3px solid #FF6B00', borderLeft: '3px solid #FF6B00' },
                      { bottom: 16, right: 16, borderBottom: '3px solid #FF6B00', borderRight: '3px solid #FF6B00' },
                    ].map((s, i) => (
                ...
                {qrLoading && (
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                    <Spin />
                    <span style={{ color: '#fff', fontSize: 11, fontFamily: "'Space Mono',monospace" }}>Verifying QR...</span>
                  </div>
                )}
              </div>

              {qrError && (
                <div style={{ color: theme.errorText, fontSize: 11, marginBottom: 12, padding: '10px 14px', background: theme.errorBg, border: `1px solid ${theme.errorBorder}`, borderRadius: 8, fontFamily: "'Space Mono',monospace" }}>
                  ⚠ {qrError}
                </div>
              )}
              ...
              {!scanning && !qrLoading && (
                <button onClick={startScanner} style={{
                  ...
                  fontFamily: "'Space Mono',monospace", fontSize: 11, letterSpacing: 2, textTransform: 'uppercase',
                  cursor: 'pointer', boxShadow: '0 8px 24px rgba(255,107,0,0.3)',
                }}>
                  📷 Start Camera
                </button>
              )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 16, padding: '10px 12px', background: 'rgba(255,107,0,0.06)', border: '1px solid rgba(255,107,0,0.15)', borderRadius: 8 }}>
                <span>💡</span>
                <span style={{ color: theme.textMuted, fontSize: 11, lineHeight: 1.5, textAlign: 'left' }}>Find your QR code on your dashboard after logging in with email once.</span>
              </div>
            </div>
          )}
