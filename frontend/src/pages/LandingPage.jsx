...
export default function LandingPage({ onNavigate }) {
  ...
  return (
    <div>
      {/* Hero */}
      <section style={{ background: mode==='dark'?'linear-gradient(160deg,#0a0a0a 0%,#111 100%)': `linear-gradient(160deg,#fff 0%, color-mix(in srgb, var(--primary) 5%, white) 100%)`, padding: isMobile?'60px 16px 48px':'100px 32px 80px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'10%', left:'50%', transform:'translateX(-50%)', width:500, height:300, background:'radial-gradient(ellipse,color-mix(in srgb, var(--primary) 8%, transparent),transparent 70%)', pointerEvents:'none' }} />
        <div style={{ position:'relative', maxWidth:700, margin:'0 auto' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'5px 14px', borderRadius:40, background:'color-mix(in srgb, var(--primary) 8%, transparent)', border:'1px solid color-mix(in srgb, var(--primary) 25%, transparent)', color:'var(--primary)', fontSize:13, fontFamily:"'Space Mono',monospace", letterSpacing:2, textTransform:'uppercase', marginBottom:24 }}>
          ...
          <h1 style={{
            ...
            border:`1px solid color-mix(in srgb, var(--primary) 35%, transparent)`, borderRadius:20,
            padding: isMobile?'28px 20px':'36px 40px',
            boxShadow:'0 8px 32px color-mix(in srgb, var(--primary) 15%, transparent)',
          }}>
            SHOP MORE.<br/><span style={{ color:'var(--primary)' }}>EARN MORE.</span><br/>REWARD YOURSELF.
          </h1>
          ...
          <div style={{ display:'flex', justifyContent:'center', gap: isMobile?24:48, marginTop:56, flexWrap:'wrap' }}>
            {[['500K+','Members'],['2.5M+','Points Earned'],['500+','Stores'],['5','Card Types']].map(([num,lbl]) => (
              <div key={lbl} style={{ textAlign:'center' }}>
                <div style={{ color:'var(--primary)', fontFamily:"'Bebas Neue',sans-serif", fontSize: isMobile?32:41, letterSpacing:2, lineHeight:1 }}>{num}</div>
