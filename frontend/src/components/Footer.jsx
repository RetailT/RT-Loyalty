    ...
    <footer style={{ background: mode==='dark'?'#0a0a0a':'#ffffff', borderTop:`1px solid ${theme.border}`, marginTop: showNav ? 48 : 0, transition:'background 0.3s' }}>
          ...
          {isMobile ? (
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:8 }}>
                {navLinks.map(l => (
                  <button key={l.page} onClick={() => onNavigate && onNavigate(l.page)} style={{ background: mode==='dark'?'#111':'#f8f8f8', border:`1px solid ${theme.border}`, borderRadius:10, padding:'10px 6px', cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                    <span style={{ fontSize:16, color:'var(--primary)' }}>{l.icon}</span>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', gap:48, alignItems:'flex-start' }}>
              <div style={{ flex:'0 0 auto', maxWidth:360 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color:'var(--primary)', fontSize:9, letterSpacing:3, textTransform:'uppercase', fontFamily:'monospace' }}>LOYALTY PORTAL</div>
                  </div>
                </div>
              ...
              <div style={{ flex:'0 0 auto' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0px 48px', alignItems:'start' }}>
                  {navLinks.map(l => (
                    <button key={l.page} onClick={() => onNavigate && onNavigate(l.page)}
                      style={{ background:'transparent', border:'none', color:theme.textMuted, fontSize:13, display:'flex', alignItems:'center', gap:10, padding:'8px 0', cursor:'pointer', transition:'color 0.2s', justifyContent:'flex-start', textAlign:'left', whiteSpace:'nowrap' }}
      ...
      <div style={{ borderTop: showNav ? `1px solid ${theme.border}` : 'none' }}>
        <div style={{ maxWidth:1100, margin:'0 auto', padding: isMobile?'14px 16px':'16px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
          <div style={{ color:theme.textFaint, fontSize: isMobile?10:12 }}>© {year} Retail Target Software Solutions (Pvt) Ltd.</div>
          <div style={{ color:theme.textFaint, fontSize: isMobile?10:12 }}>Powered by <span style={{ color:'var(--primary)' }}>RT POS</span></div>
        </div>
      </div>
    </footer>
  );
