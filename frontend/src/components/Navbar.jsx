    ...
    <>
      <nav style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }} onClick={() => handleNav(user ? 'dashboard' : 'landing')}>
          <div style={{ width:34, height:34, background:'linear-gradient(135deg,#FF6B00,#FF8C00)', borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:900, color:'#fff', boxShadow:'0 4px 12px rgba(255,107,0,0.35)', flexShrink:0 }}>R</div>
          <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap: isMobile?8:12 }}>
          ...
          {user ? (
            <>
              ...
              <div style={{ position:'relative' }}>
                ...
                {menuOpen && (
                  <div style={{ position:'absolute', top:42, right:0, background:theme.bgCard, border:`1px solid ${theme.border}`, borderRadius:12, overflow:'hidden', minWidth:180, boxShadow:'0 8px 32px rgba(0,0,0,0.15)', zIndex:200 }}>
          ...
          {isTablet && user && (
            <button onClick={() => setTabletNav(!tabletNav)} style={{ background:theme.bgAccent, border:`1px solid ${theme.border}`, borderRadius:8, padding:'7px 10px', cursor:'pointer', color:theme.text, fontSize:16 }}>
              {tabletNav ? '✕' : '☰'}
            </button>
          )}
        </div>
