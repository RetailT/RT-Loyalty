// controllers/portalCustomerController.js — POSBACK_CODE filtered
...
exports.getMe = async (req, res) => {
  try {
    ...
    if (!result.recordset.length) return res.status(404).json({ success: false, message: 'Customer not found.' });
...
exports.getTransactions = async (req, res) => {
  try {
    ...
    const type  = req.query.type || 'all'; // 'earn' | 'redeem' | 'all'
    ...
    const result  = await posPool.request()
      ...
      .query(`
        SELECT IDX, ID, RATE, AMOUNT, INVOICE_DATE, INVOICE_TIME, INVOICENO,
               COMPANY_NAME, LOYALTY_TYPE, CURRATE
        FROM (
  ...
  try {
    ...
    const [rewardsRes, pointsRes] = await Promise.all([
      loyPool.request()
        ...
        .query(`
          SELECT IDX, TITLE, DESCRIPTION, CATEGORY, POINTS_COST, ICON, IS_ACTIVE, VALID_FROM, VALID_UNTIL, STOCK
          FROM dbo.tb_REWARDS
          WHERE IS_ACTIVE = 1
    ...
    res.json({ success: true, data: rewardsRes.recordset, availablePoints: pointsRes.availablePoints });
  } catch (err) {
...
exports.redeemReward = async (req, res) => {
  try {
    ...
    const rwRes = await loyPool.request()
      ...
      .query(`SELECT TOP 1 IDX, TITLE, POINTS_COST, IS_ACTIVE FROM dbo.tb_REWARDS WHERE IDX = @rid AND IS_ACTIVE = 1`);
    if (!rwRes.recordset.length) return res.status(404).json({ success: false, message: 'Reward not found.' });

    const reward = rwRes.recordset[0];
    const points = await getPointsSummary(posPool, serialNo, companyCode);

    if (points.availablePoints < reward.POINTS_COST) {
      return res.status(400).json({ success: false, message: `Not enough points. Need ${reward.POINTS_COST}, have ${points.availablePoints.toFixed(2)}.` });
    }

    // Get customer IDX
    const posPool2  = await getPosbackPool();
    const custRes2  = await posPool2.request()
      .input('sno',  sql.NVarChar, serialNo)
      .input('code', sql.Char,     companyCode)
      .query(`SELECT TOP 1 IDX FROM dbo.tb_LOYALTYCUSTOMER_MAIN WHERE SERIALNO = @sno AND COMPANY_CODE = @code`);
    if (!custRes2.recordset.length) return res.status(404).json({ success: false, message: 'Customer not found.' });
    const custIdx2 = custRes2.recordset[0].IDX;

    // Insert redemption record
    await loyPool.request()
      .input('custId', sql.Int,   custIdx2)
      .input('rid',    sql.Int,   reward.IDX)
      .input('pts',    sql.Float, reward.POINTS_COST)
      .query(`INSERT INTO dbo.tb_REDEMPTIONS (CUSTOMER_ID, REWARD_ID, POINTS_COST, STATUS, REDEEMED_AT) VALUES (@custId, @rid, @pts, 'PENDING', GETDATE())`);

    // Write RD row to POSBACK
    await posPool.request()
      .input('sno',   sql.NVarChar, serialNo)
      .input('code',  sql.Char,     companyCode)
      ...
      .query(`
        INSERT INTO dbo.tb_LOYALTY_TRANSACTION
          (SERIALNO, COMPANY_CODE, CUSTOMER_NAME, ID, RATE, AMOUNT, INVOICENO, INVOICE_DATE, INVOICE_TIME, SMS, LOYALTY_TYPE, CURRATE)
        VALUES
          (@sno, @code, @name, 'RD', @rate, 0, @desc, GETDATE(), CONVERT(VARCHAR(8),GETDATE(),108), 'T', '', 0)
      `);
